...
namespace BTX
{
	...
	*/
		...
		*/
		...
		/*
		USAGE: Generates a Circle mesh using tris 
		ARGUMENTS:
		-	float a_fRadius -> radius of the circle
		-	vector3 a_v3Origin -> the origin from where to draw the circle
		-	int a_nnumPoints -> Number of points on the circle
		-	a_v3Color -> Color of the mesh
		OUTPUT: ---
		*/
		void GenerateCircle(float a_fRadius, vector3 a_v3Origin, int a_nnumPoints, vector3 a_v3Color = C_WHITE);
	};
}
